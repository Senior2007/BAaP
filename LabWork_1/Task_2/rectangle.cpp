...
Rectangle::Rectangle(const QPoint& start, const QPoint& end): Polygon(QPoint(0, 0), start), m_end(end) {
...
void Rectangle::draw(QPainter& painter, const Shape* selectedShape) const {
    painter.save();
    painter.translate(m_start); // Перемещаем начало координат в центр фигуры
...
void Rectangle::showInformation(QPainter& painter, int height) {
    ...
    QPoint Center = QPoint(m_start.x() + (m_end.x() - m_start.x()) * SF / 2, m_start.y() + (m_end.y() - m_start.y()) * SF / 2);
    QTransform transform;
    transform.translate(m_start.x(), m_start.y());
    transform.rotate(rotation);
    transform.translate(-m_start.x(), -m_start.y());
    Center = transform.map(Center);

    QRect rect5(250, height - 22, 100, 20);
    painter.drawText(rect5, Qt::AlignLeft, QString("ЦМ X: ") + QString::number(Center.x(), 'f', 2));

    QRect rect6(250, height - 43, 100, 20);
    painter.drawText(rect6, Qt::AlignLeft, QString("ЦМ Y: ") + QString::number(Center.y(), 'f', 2));
}

...
void Canvas::mouseMoveEvent(QMouseEvent* event) {
    ...
    if (m_selectedShape && (event->modifiers() & Qt::ShiftModifier)) {
        ...
        if (m_selectedShape) m_selectedShape->scale(scaleFactor);
        m_currentPos = event->pos();
        update();
    }
    else {
        if(event->modifiers() & Qt::ControlModifier){
            double x = event->pos().x() - m_currentPos.x();
            double y = event->pos().y() - m_currentPos.y();
            double angle = qAtan2(y, x);
            if (m_selectedShape) m_selectedShape->rotate(angle);
            m_currentPos = event->pos();
            update();
        } else if (m_isDrawing) {
            m_currentPos = event->pos();
